...
export default function UploadForm() {
  ...
  const connectWebSocket = (jobId: string) => {
    ...
    ws.onopen = () => {
      ...
      setMessage({ text: "Subscribed. Waiting for processing to start...", type: "info" });
      ...
      try {
        ...
        switch (data.status) {
          ...
          case "Completed":
            ...
            setResult({ // Update result data with final details
                fileName: selectedFile?.name ?? "Unknown", // Get filename from state
                price: data.price,
                estimatedTime: data.printTimeSeconds ? formatDuration(data.printTimeSeconds) : undefined,
                materialUsage: data.filamentUsedGrams ? `${data.filamentUsedGrams.toFixed(2)} g` : undefined
            });
          ...
          case "Failed": // Handle potential failure status from backend
            setStage("failed");
            setMessage({ text: `Processing failed: ${data.errorMessage || 'Unknown error'}`, type: "error" });
    ...
    ws.onerror = (error) => {
      ...
      setMessage({ text: "WebSocket connection error. Cannot get real-time updates.", type: "error" });
      setStage("failed");
      setIsProcessing(false);
    };
    ...
    ws.onclose = () => {
      ...
      if (stage !== 'completed') {
           ...
           if (isProcessing) {
             setStage("failed");
             setMessage({ text: "Connection lost during processing.", type: "error"});
             setIsProcessing(false);
           }
      }
    };
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-6 w-full max-w-lg mx-auto p-4">
      ...
      >
        {/* File drop zone */}
        <label className={`flex flex-col items-center justify-center w-full p-8 border-2 border-dashed rounded-2xl transition relative ${
             isProcessing ? 'cursor-not-allowed bg-gray-100 border-gray-300' : 'cursor-pointer bg-blue-50 hover:bg-blue-100 border-blue-400'
        }`}>
          <svg
            className={`w-10 h-10 mb-2 ${isProcessing ? 'text-gray-400' : 'text-blue-600'}`}
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            viewBox="0 0 24 24"
          ...
          <span className={`font-medium text-center ${isProcessing ? 'text-gray-500' : 'text-gray-800'}`}>
            {selectedFile
              ? selectedFile.name
        ...
        {isProcessing && (
          <div className="w-full mt-2">
            ...
            <div className="text-center text-sm text-gray-600 mt-1">
               {stageLabels[stage]}
               {(stage === 'processing' && message.type === 'info') && ` (${message.text})`}
            </div>
          </div>
        )}
        ...
        {message.text && !isProcessing && (
            <div className={`mt-2 px-4 py-2 rounded-lg text-sm font-medium transition w-full text-center ${
      ...
      {result && stage === "completed" && (
        <div className="w-full p-5 mt-4 border border-gray-200 rounded-xl bg-gray-50 shadow-sm">
          ...
          <ul className="text-sm text-gray-700 space-y-1">
            ...
            {result.estimatedTime && <li><strong>Estimated Print Time:</strong> {result.estimatedTime}</li>}
            {result.materialUsage && <li><strong>Material Usage:</strong> {result.materialUsage}</li>}
            {/* <li><strong>Layer Count:</strong> {result.layerCount ?? 'N/A'}</li> */}
            {result.price !== undefined && <li><strong>Estimated Price:</strong> {result.price.toFixed(2)} RSD</li>}
          </ul>
        </div>
      )}
    </div>
